...
public class DateUtil {
    ...
    /**
     * 要计算的区时=已知区时-(已知区时的时区-要计算区时的时区)； 东时区为正，西时区为负
     *
     * @param srcHour    已知区时
     * @param srcZone    已知区时的时区
     * @param targetZone 要计算区时的时区
     * @return
     */
    public static OffsetTime offsetTime(int srcHour, int srcZone, int targetZone) {

        OffsetTime offsetTime = new OffsetTime();

        int hour = srcHour - (srcZone - targetZone);
        if (hour < 0) {

            offsetTime.setHour(hour + 24);
            offsetTime.setOffsetDay(-1);
        } else if (hour > 24) {

            offsetTime.setHour(hour % 24);
            offsetTime.setOffsetDay(hour / 24);
        } else {
